...
TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#rotationList').bootstrapTable('destroy');
        $('#rotationList').bootstrapTable({
            url: "/panelcontent/list?&rdm=" + Math.random(), //请求后台的URL（*）
            method: 'GET',                     //请求方式（*）
            dataType:'json',
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    ...
                    formatter: function (value, row, index) {
                        console.log(row);
                        return showPanelPicture(row.imageUrl);
                    }
                }
                , {
                    field: 'activeId',
...
/*轮播图-启用 禁用 */
function rotation_status_change(id,newStatus,oldStatus){
    var msg;
    var tmpMsg;
    if (newStatus == 0) {
        msg = "确定启用吗？";
    ...
    layer.confirm(msg,function(){
        $.ajax({
            type: 'POST',
            url: '/page/rotation/editRotationStatus',
            dataType: 'json',
            data:{
                id:id,
                status:newStatus
            ...
            success: function(data){
                $('#rotationList').bootstrapTable('refresh');
                layer.msg(tmpMsg,{icon: 5,time:1000});
            },
            error:function(data) {
...
/*轮播图的删除*/
function delete_rotation(ids) {
    layer.confirm("确定删除吗？",function() {
       $.ajax({
           type: 'POST',
           url: '/page/rotation/deleteRotation',
           dataType: 'json',
           data:{
               ids:ids,
           },
...
function batchDelete() {
    var selectEl = $("input[name='isSelect']:checked");
    console.log(selectEl);
    var ids = [];
    for (var i=0 ; i < selectEl.length; i ++) {
        console.log(selectEl[i]);
        ids.push(selectEl[i].value);
    }
    console.log(ids);
    delete_rotation(ids.join(","));
}
